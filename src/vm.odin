...
run_program :: proc(c: []^Compiled_Module, entry_point: int) {
	...
	run: for {
		...
		case .Op_Call_Method:
			method_addr := get_i16(&vm)
			val := pop_stack_value(&vm)
			instance := cast(^Class_Object)val.data.(^Object)
			push_call_frame(&vm, &instance.vtable.methods[method_addr].chunk)

		case .Op_Call_Constr:
			method_addr := get_i16(&vm)
			val := pop_stack_value(&vm)
			prototype := cast(^Class_Object)val.data.(^Object)
		...
		case .Op_Bind:
			var_addr := get_i16(&vm)
			stack_addr := int(get_i16(&vm)) + get_scope_start_id(&vm)
			vm.chunk.variables[var_addr].stack_id = stack_addr

		case .Op_Set:
			var_addr := get_i16(&vm)
			stack_addr := vm.chunk.variables[var_addr].stack_id
			set_stack_value(&vm, stack_addr, pop_stack_value(&vm))
